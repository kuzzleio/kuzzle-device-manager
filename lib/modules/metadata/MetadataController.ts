...
import {
  MetadataContent,
  ProcessedMetadataContent,
} from "./types/MetadataContent";
...
export class MetadataController extends RelationalController {
    ...
    this.definition = {
      actions: {
        ...
        delete: {
          ...
          http: [
            { path: "device-manager/:engineId/metadata/:_id", verb: "delete" },
          ],
        },
        get: {
          handler: this.get.bind(this),
          http: [
            { path: "device-manager/:engineId/metadata/:_id", verb: "get" },
          ],
        },
        update: {
          handler: this.update.bind(this),
          http: [
            { path: "device-manager/:engineId/metadata/:_id", verb: "put" },
          ],
        },
      },
    };
    ...
    if (objectValueList) {
      ...
      for (const objectValue of objectValueList) {
        const formattedMetadataValues =
          await this.assetCategoryService.formatMetadataForES(objectValue);
  ...
  async delete(request: KuzzleRequest) {
    const AssetCategory = this.getFieldPath(
      request,
      "assetMetadata",
      null,
      "asset-category"
  ...
  async get(request: KuzzleRequest) {
    ...
    const metadata = await this.sdk.document.get<MetadataContent>(
      engineId,
      this.collection,
      id
    );
    const processedMetadata = JSON.parse(
      JSON.stringify(metadata._source)
    ) as ProcessedMetadataContent;
    ...
    if (metadata._source.objectValueList) {
      ...
      for (const objectValue of metadata._source.objectValueList) {
        processedMetadata.objectValueList.push(
          this.assetCategoryService.formatMetadataForGet(objectValue.object)
        );
      }
    }

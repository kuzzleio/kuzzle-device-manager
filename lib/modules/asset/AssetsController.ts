...
export class AssetsController {
  ...
  async linkDevices(
    ...
    const measuresTolink = request.getBodyArray(
      "linkedMeasures",
      [],
    ) as ApiAssetlinkDevicesRequest["body"]["linkedMeasures"];
    ...
    for (const measure of measuresTolink) {
      const { deviceId, implicitMeasuresLinking, measureSlots } = measure;
      ...
      devices.push(DeviceSerializer.serialize(device));
      asset = updatedAsset;
    }
    return {
  ...
  async unlinkDevices(
    ...
    const deviceIds = request.getBodyArray("devices", []);
    const measureSlots = request.getBodyArray("measureSlots", []);
    if (!allMeasures && deviceIds.length === 0 && measureSlots.length === 0) {
      throw new BadRequestError(
        `The list of measures to unlink from asset ${assetId} is empty`,
      );
    }
    return this.assetService.unlinkDevices(
      assetId,
      allMeasures,
      deviceIds,
      measureSlots,

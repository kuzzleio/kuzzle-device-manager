...
export abstract class RelationalController extends CRUDController {
  public static classMap: Map<string, RelationalController> = new Map<
    string,
    RelationalController
  >(); //key : collection name (must be index+collection in futur....). Value : controller
  ...
  async genericUpdate(
    request: KuzzleRequest,
    manyToManyLinkedFields?: string[],
    oneToManyLinkedFields?: string[]
  ) {
    request.input.args.index = request.getString("engineId");
    ...
    if (manyToManyLinkedFields || oneToManyLinkedFields) {
      ...
      if (manyToManyLinkedFields) {
        for (const linkedField of manyToManyLinkedFields) {
          promises.push(
            this.propagateUpdate(linkedField, true, document, request)
          );
        }
      }
      if (oneToManyLinkedFields) {
        for (const linkedField of oneToManyLinkedFields) {
          promises.push(
            this.propagateUpdate(linkedField, false, document, request)
          );
        }
      }
    }
  ...
  async propagateUpdate(
    linkedField: string,
    manyToMany: boolean,
    documentToUpdate: KDocument<any>,
    originalRequest: KuzzleRequest
  ) {
    ...
    for (const containerFieldPath of listContainer) {
      ...
      //Lazy deleting
      const containerDocument = await this.getDocumentContent(
        containerFieldPath,
        false
      );
      ...
      if (manyToMany) {
        ...
        const indexToEdit = arrayToEdit.findIndex(
          (element) => element._kuzzleId === updateId
        );
      ...
      promises.push(
        this.updateRequest(containerFieldPath, containerUpdateBody, user)
      );
    }
    await Promise.all(promises);
  }
  ...
  async genericDelete(
    ...
    if (manyToManyLinkedFields.length > 0 || oneToManyLinkedFields.length > 0) {
      ...
      for (const childrenField of manyToManyLinkedFields) {
        promises.push(
          this.propagateDelete(
            document._source[childrenField],
            childrenField,
            true,
            request
          )
      ...
      for (const containerField of oneToManyLinkedFields) {
        promises.push(
          this.propagateDelete(
            document._source[containerField],
            containerField,
            false,
            request
          )
   ...
   * @param nestedFields : contain field name, collection name and index name of documents that can have a reference to the document to delete.
   */
  async deleteNested(request: KuzzleRequest, nestedFields: FieldPath[] = []) {
    //nestedField must contain : index/collection and field with id to object to delete of CONTAINER
    request.input.args.index = request.getString("engineId");
    const promises: Promise<void>[] = [];
    for (const nestedField of nestedFields) {
      ...
      promises.push(
        this.sdk.document
          .search(nestedField.index, nestedField.collection, search)
          .then((find) => {
   ...
   */
  async propagateToNested(
    documents: SearchResult<KHit<KDocumentContentGeneric>>,
    index: string,
    collection: string,
    field: string,
    requestId: string,
    ...
    while (documents) {
      for (const document of documents.hits) {
        ...
        if (Array.isArray(document._source[field])) {
          request[field] = document._source[field].filter(
            (id) => id !== requestId
          );
        } else {
          request[field] = null;
        ...
        promises.push(
          this.updateRequestRaw(index, collection, document._id, request, user)
        );
      }
  ...
  async genericLink(
    ...
    if (manyToMany) {
      ...
      updateMessageDest[container.field] = containerDocument[container.field]
        ? containerDocument[container.field]
        : [];
  ...
  async genericUnlink(
    ...
    const document = await this.getDocumentContent(embedded);
    if (!document[embedded.field]) {
      global.app.errors.get(
        "device-manager",
        "relational_controller",
        "remove_unexisting_link"
      );
    }
    const index = document[embedded.field].findIndex((fieldPath) =>
      this.equal(fieldPath, container)
    );
    if (index === -1) {
      global.app.errors.get(
        "device-manager",
    ...
    const updateRequest = {};
    if (manyToMany) {
      updateRequest[container.field] = containerDocument[
        container.field
      ].filter(
        (embeddedDocument) => embeddedDocument._kuzzleId !== embedded.document
      );
    } else {
  ...
  private removeLink(
    ...
    containerDocument: KDocument<KDocumentContentGeneric>
  ) {
    if (!request.input.body[linkedField]) {
      request.input.body[linkedField] = containerDocument._source[linkedField];
    }
    request.input.body[linkedField] = request.input.body[linkedField].filter(
      (field) => !this.equal(field, containerFieldPath)
    );
  }
   ...
   * @private
   */
  private async updateRequestRaw(
    index: string,
    collection: string,
    document: string,
    body,
    ...
    if (
      RelationalController.classMap &&
      RelationalController.classMap.has(collection)
    ) {
      ...
      await this.sdk.document.update(index, collection, document, body);
    }
  }
   ...
   */
  protected async genericGet<TKDocumentContent extends KDocumentContent>(
    index: string,
    collection: string,
    documentId: string,
    nestedFields: FieldPath[] = []
  ): Promise<KDocument<TKDocumentContent>> {
    const document = await this.sdk.document.get<TKDocumentContent>(
      index,
      collection,
      documentId
    );
    return this.esDocumentToFormatted(
      index,
      collection,
      document,
      nestedFields
   ...
   * @param nestedFields : contain field name in the document to get, collection name and index name of documents that are linked
   */
  protected async esDocumentToFormatted<
   ...
   */
  private async replaceNestedFieldByContent(
    document: KDocument<KDocumentContent>,
    nestedField: FieldPath
  ) {
    if (document._source[nestedField.field]) {
      document._source[nestedField.field] = await this.getRequestRaw(
        nestedField.index,
        nestedField.collection,
        document._source[nestedField.field]
      );
    }
  }

  private async getRequestRaw(
    index: string,
    collection: string,
    documentId: string
  ): Promise<KDocumentContent> {
    const request = new KuzzleRequest(
      {
        _id: documentId,
        collection,

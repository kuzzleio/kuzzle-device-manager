...
export class AssetCategoryController extends RelationalController {
    ...
    this.definition = {
      actions: {
        get: {
          ...
          http: [
            {
              path: "device-manager/:engineId/assetCategory/:_id",
              verb: "get",
            },
          ],
        },
        create: {
          ...
          http: [
            { path: "device-manager/:engineId/assetCategory", verb: "post" },
          ],
        },
        delete: {
        ...
        linkMetadata: {
          ...
          http: [
            {
              path: "device-manager/:engineId/assetCategory/:_id/_link/metadata/:metadataId",
              verb: "put",
            },
          ],
        },
        unlinkMetadata: {
          ...
          http: [
            {
              path: "device-manager/:engineId/assetCategory/:_id/_unlink/metadata/:metadataId",
              verb: "delete",
            },
          ],
        },
        linkParent: {
          ...
          http: [
            {
              path: "device-manager/:engineId/assetCategory/:_id/_link/parent/:parentId",
              verb: "put",
            },
          ],
        },
        unlinkParent: {
          ...
          http: [
            {
              path: "device-manager/:engineId/assetCategory/:_id/_unlink/parent/:parentId",
              verb: "delete",
            },
          ],
        },
  ...
  async update(request: KuzzleRequest) {
    ...
    if (assetMetadata) {
      ...
      if (!request.input.body.metadataValues) {
        const document = await this.sdk.document.get<AssetCategoryContent>(
          engineId,
          this.collection,
          id
        );
      ...
      let index = 0;
      const metadataValues = request.input.body
        .metadataValues as FormattedMetadata[];
  ...
  async unlinkMetadata(request: KuzzleRequest) {
    ...
    const category = await this.sdk.document.get<AssetCategoryContent>(
      engineId,
      "asset-category",
      id
    );
    const updatedMetadata = category._source.assetMetadata.filter(
      (ownedMetadataId) => ownedMetadataId !== metadataId
    );
    request.input.body = {
  ...
  async linkMetadata(request: KuzzleRequest) {
    ...
    const [metadataContent, category] = await Promise.all([
      await this.sdk.document.get<MetadataContent>(
        engineId,
        "metadata",
        metadataId
      ),
      await this.sdk.document.get<AssetCategoryContent>(
        engineId,
        "asset-category",
        id
      ),
    ]);
    const metadata = category._source.assetMetadata
      ? category._source.assetMetadata
      : [];
    ...
    let update = false;
    if (value) {
      this.assetCategoryService.validateEnumMetadata(
        metadataContent._source,
        value
      );
      const formattedValue: FormattedValue =
        this.assetCategoryService.formatValue(value);
      if (metadataValues) {
  ...
  async get(request: KuzzleRequest): Promise<ProcessedAssetCategoryContent> {
    ...
    const formattedAssetMetadata =
      this.assetCategoryService.formatMetadataForGet(metadataValues);
    const children = this.getFieldPath(request, "parent");
    const processedCategory = await this.genericGet<AssetCategoryContent>(
      engineId,
      this.collection,
      id,
      [children]

...
> **Terminology note:** In the codebase, "engine" and "tenant" are synonyms. An `engineId` is a tenant identifier, an `engineGroup` is a tenant group. Specs use domain language (tenant/tenantId), implementation uses code language (engine/engineId). The new field is `engines?: string[]` in code (tenant IDs in domain).

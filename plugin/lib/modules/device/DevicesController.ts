...
export class DevicesController {
  ...
  constructor(
    ...
    this.definition = {
      actions: {
        ...
        update: {
          ...
          http: [{ path: 'device-manager/:engineId/devices/:_id', verb: 'put' }],
        },
        upsert: {
    ...
    this.exporter = new DigitalTwinExporter(this.plugin, InternalCollection.DEVICES);
    this.measureExporter = new MeasureExporter(this.plugin, InternalCollection.DEVICES);
  }
  ...
  async update(request: KuzzleRequest): Promise<ApiDeviceUpdateResult> {
    ...
    const updatedDevice = await this.deviceService.update(engineId, deviceId, metadata, request);

    return DeviceSerializer.serialize(updatedDevice);
  }
  async replaceMetadata(request: KuzzleRequest): Promise<ApiDeviceMetadataReplaceResult> {
    const deviceId = request.getId();
    const engineId = request.getString('engineId');
    const metadata = request.getBodyObject('metadata');
  ...
  async create(request: KuzzleRequest): Promise<ApiDeviceCreateResult> {
    ...
    const device = await this.deviceService.create(model, reference, metadata, request);
   ...
   */
  async attachEngine(request: KuzzleRequest): Promise<ApiDeviceAttachEngineResult> {
    const engineId = request.getString('engineId');
    const deviceId = request.getId();
   ...
   */
  async detachEngine(request: KuzzleRequest): Promise<ApiDeviceDetachEngineResult> {
    const deviceId = request.getId();
   ...
   */
  async unlinkAsset(request: KuzzleRequest): Promise<ApiDeviceUnlinkAssetResult> {
    const deviceId = request.getId();

    const { asset, device } = await this.deviceService.unlinkAsset(deviceId, request);
  ...
  async getMeasures(request: KuzzleRequest): Promise<ApiDeviceGetMeasuresResult> {
  ...
  async getLastMeasures(request: KuzzleRequest): Promise<ApiDeviceGetLastMeasuresResult> {
    const deviceId = request.getId();
    const engineId = request.getString('engineId');
    const measureCount = request.getNumber('measureCount', 100);

    const results = await this.deviceService.getLastMeasures(engineId, deviceId, measureCount);

    return results.reduce<ApiDeviceGetLastMeasuresResult>((accumulator, result) => {
  ...
  async mGetLastMeasures(request: KuzzleRequest): Promise<ApiDeviceMGetLastMeasuresResult> {
    const engineId = request.getString('engineId');
    const measureCount = request.getNumber('measureCount', 100);
    const deviceIds = request.getBodyArray('ids');

    const results = await this.deviceService.mGetLastMeasures(engineId, deviceIds, measureCount);
  ...
  async receiveMeasures(request: KuzzleRequest) {
    ...
    await this.deviceService.receiveMeasures(engineId, deviceId, measures, payloadUuids, request);
  }
  ...
  async getLastMeasuredAt(request: KuzzleRequest): Promise<ApiDeviceGetLastMeasuredAtResult> {
    const deviceId = request.getId();
    const engineId = request.getString('engineId');

    const lastMeasuredAt = await this.deviceService.getLastMeasuredAt(engineId, deviceId);
  ...
  async mGetLastMeasuredAt(request: KuzzleRequest): Promise<ApiDeviceMGetLastMeasuredAtResult> {
    const engineId = request.getString('engineId');
    const deviceIds = request.getBodyArray('ids');

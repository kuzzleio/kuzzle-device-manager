...
export class AssetModule extends Module {
  ...
  constructor(plugin: DeviceManagerPlugin) {
    ...
    this.assetsGroupsLogger = this.plugin.context.logger.child('assetsGroups-module');
  }

  public async init(): Promise<void> {
    this.assetHistoryService = new AssetHistoryService(this.plugin, this.logger);
    this.assetService = new AssetService(this.plugin, this.assetHistoryService, this.logger);
    this.assetController = new AssetsController(this.plugin, this.assetService, this.logger);
    this.assetsGroupsService = new AssetsGroupsService(this.plugin, this.assetsGroupsLogger);
    ...
    this.plugin.api['device-manager/assetsGroup'] = this.assetGroupsController.definition;
    ...
    this.plugin.imports.roles[RoleAssetsGroupsAdmin.name] = RoleAssetsGroupsAdmin.definition;
    this.plugin.imports.roles[RoleAssetsGroupsReader.name] = RoleAssetsGroupsReader.definition;
  }
}

...
import { MeasureValidationError, MeasureValidationChunks } from '../measure/MeasureValidationError';
...
export class AssetsController {
  ...
  constructor(
    ...
    this.exporter = new DigitalTwinExporter(this.plugin, InternalCollection.ASSETS);
    this.measureExporter = new MeasureExporter(this.plugin, InternalCollection.ASSETS);
  }
  ...
  async update(request: KuzzleRequest): Promise<ApiAssetUpdateResult> {
    ...
    const updatedAsset = await this.assetService.update(engineId, assetId, metadata, request);
  ...
  async create(request: KuzzleRequest): Promise<ApiAssetCreateResult> {
    ...
    const asset = await this.assetService.create(engineId, model, reference, metadata, request);
  ...
  async getMeasures(request: KuzzleRequest): Promise<ApiAssetGetMeasuresResult> {
  ...
  async getLastMeasures(request: KuzzleRequest): Promise<ApiAssetGetLastMeasuresResult> {
    const assetId = request.getId();
    const engineId = request.getString('engineId');
    const measureCount = request.getNumber('measureCount', 100);

    const results = await this.assetService.getLastMeasures(engineId, assetId, measureCount);

    return results.reduce<ApiAssetGetLastMeasuresResult>((accumulator, result) => {
  ...
  async mGetLastMeasures(request: KuzzleRequest): Promise<ApiAssetMGetLastMeasuresResult> {
    const engineId = request.getString('engineId');
    const measureCount = request.getNumber('measureCount', 100);
    const assetIds = request.getBodyArray('ids');

    const results = await this.assetService.mGetLastMeasures(engineId, assetIds, measureCount);

    const response: ApiAssetMGetLastMeasuresResult = {};

    for (const [assetId, measures] of Object.entries(results)) {
      response[assetId] = measures.reduce<ApiAssetGetLastMeasuresResult>((accumulator, result) => {
        const measure: EmbeddedMeasure = {
          measuredAt: result.measuredAt,
          name: result.asset.measureName,
          originId: result.origin._id,
  ...
  private async getTypeFromMeasureSlot(
    ...
    try {
      const assetDocument = await this.plugin.context.accessors.sdk.document.get<AssetContent>(
        indexId,
    ...
    return assetModel.asset.measures.find((elt) => elt.name === measureName)?.type ?? null;
  }
  ...
  async mMeasureIngest(request: KuzzleRequest) {
    ...
    if (!isSourceApi(source)) {
      throw new BadRequestError('The provided data source does not match the API source format');
    }
  ...
  async measureIngest(request: KuzzleRequest) {
    ...
    const type = await this.getTypeFromMeasureSlot(indexId, assetId, measureName, engine.group);

    if (!type) {
      throw new BadRequestError(`Slot name ${measureName} does not exist on Asset ${assetId}`);
    }
    ...
    if (validator) {
      ...
      if (!valid) {
        throw new MeasureValidationError('The provided measure does not respect its schema', [
          {
  ...
  async exportMeasures(request: KuzzleRequest) {
    ...
    const link = await this.measureExporter.prepareExport(engineId, request.getUser(), {
      endAt,
      id,
      lang,
      query,
  ...
  async export(request: KuzzleRequest) {
    ...
    const link = await this.exporter.prepareExport(engineId, request.getUser(), {
      lang,
      query,
      sort,
    });
  ...
  async migrateTenant(request: KuzzleRequest): Promise<ApiAssetMigrateTenantResult> {
  ...
  async getLastMeasuredAt(request: KuzzleRequest): Promise<ApiAssetGetLastMeasuredAtResult> {
    const assetId = request.getId();
    const engineId = request.getString('engineId');

    const lastMeasuredAt = await this.assetService.getLastMeasuredAt(engineId, assetId);
  ...
  async mGetLastMeasuredAt(request: KuzzleRequest): Promise<ApiAssetMGetLastMeasuredAtResult> {
    const engineId = request.getString('engineId');
    const assetIds = request.getBodyArray('ids');
   ...
   * @param request
   */
  async updateModelLocales(request: KuzzleRequest): Promise<ApiAssetUpdateModelLocales[]> {

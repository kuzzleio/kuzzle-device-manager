...
describe('features/Device/Controller/AttachEngine', () => {
  const sdk = useSdk();
  ...
  it('Attach a device to an engine and historize measures', async () => {
    ...
    response = await sendPayloads(sdk, 'dummy-temp', [{ deviceEUI: 'detached1', temperature: 21 }]);

    await sdk.collection.refresh('engine-kuzzle', 'measures');
    await expect(sdk.document.count('engine-kuzzle', 'measures')).resolves.toBe(2);
  });

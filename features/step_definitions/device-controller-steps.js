...
When(
  "I succesfully execute {string}:{string} while exeding documentsWriteCount limit",
  async function (controller, action) {
    const records = [];
    for (let i = 0; i < devicesTests.length; i++) {
      records.push({
        deviceId: `DummyTemp-detached-${i}`,
...
Then(
  "All devices in {string} {string} have the property {string} to {string}",
  async function (index, collection, key, value) {
    const deviceIds = [];
    for (let i = 0; i < devicesTests.length; i++) {
      deviceIds.push(`DummyTemp-detached-${i}`);
    }

    const { successes, errors } = await this.sdk.document.mGet(
      index,
      collection,
      deviceIds
    );
    ...
    for (let i = 0; i < successes.length; i++) {
      ...
      if (_source[key] !== theValue) {
        throw new Error(
          `engineId should be ${value} but current value is: ${_source.engineId}`
        );
      }
    }
  }
...
Then(
  /All documents "(.*?)":"(.*?)" (does not)? exists/,
  async function (index, collection, not) {
    const deviceIds = [];
    for (let i = 0; i < devicesTests.length; i++) {
      deviceIds.push(`DummyTemp-detached-${i}`);
    }

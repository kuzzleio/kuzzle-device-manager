...
export class DummyTempWithDelayMultipleDeviceDecoder extends Decoder {
  constructor () {
    super('DummyTempWithDelayMultipleDevice', ['innerTemp', 'extTemp', 'batteryLevel']);
  ...
  async decode (payload: JSONObject, request: KuzzleRequest): Promise<DecodedPayload> {
    ...
    for (const devicePayload of payload.payloads) {
      ...
      const battery: BatteryMeasurement = {
        deviceMeasureName: 'batteryLevel',
        measuredAt: Date.now(),
        type: 'battery',
        values: {
